...
public class ArticleContentDao {
	//ArticleContent 객체를 받아서 db article_content  테이블에 저장
	...
	throws SQLException {
		...
		try {
			...
			//db 작업성공하면 매개변수로 받았던 ArticleContent 객체를 반환
			if (insertedCount > 0) {
				return content;
			} else {
				return null;
	...
	//게시글 번호로 article_content 테이블 조회하여 ArticleContent 객체로 만들어서 반환
	public ArticleContent selectById(Connection conn, int no) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				//조회한 데이터를 ArticleContent 객체로 만들어서 반환
